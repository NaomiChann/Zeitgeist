...
var loading_screen_scene : String = "res://assets/menus/title/loading_screen.tscn"
...
func DeferredChangeScene( path : NodePath, target_id : int, spawn_point : Vector2 ):
	...
	var i = 1
	var dict_key = current_scene.name
	
	if pickups.size() > 0 and !world_state.scene_states.has( dict_key ):
		world_state.scene_states[dict_key] = []
	
	for pickup in pickups:
		pickup.id = i
		i += 1
		if pickup.id in world_state.scene_states[dict_key]:
			pickup.queue_free()
...
func ReturnToTitle( path : NodePath ):
	...
	world_state.scene_states.clear()
	bgm_player.stop()
